...
import ru.karatel.modules.settings.impl.StringSetting;
...
public class StringComponent extends Component {
    ...
    public void render(MatrixStack stack, float mouseX, float mouseY) {
        ...
        drawTextWithLineBreaks(stack, textToDraw + (typing && text.length() < 59 && System.currentTimeMillis() % 1000 > 500 ? "_" : ""), x + 1, y + Fonts.pro[15].getFontHeight() / 2, width - 1);
    ...
    private void drawTextWithLineBreaks(MatrixStack stack, String text, float x, float y, float maxWidth) {
        ...
        for (String line : lines) {
            ...
            for (String wrappedLine : wrappedLines) {
                ...
                Fonts.pro[15].drawString(stack, wrappedLine, x, currentY, ColorUtils.rgba(255, 255, 255, 64));
                currentY += Fonts.pro[15].getFontHeight();
            }
        }
    }
    ...
    private List<String> wrapText(String text, float size, float maxWidth) {
        ...
        for (String word : words) {
            if (Fonts.pro[15].getWidth(word) <= maxWidth) {
                if (Fonts.pro[15].getWidth(currentLine.toString() + word) <= maxWidth) {
    ...
    private StringBuilder breakAndAddWord(String word, StringBuilder currentLine, float size, float maxWidth, List<String> lines) {
        ...
        for (int i = 0; i < wordLength; i++) {
            ...
            if (Fonts.pro[15].getWidth(nextPart) <= maxWidth) {
    ...
    private float calculateHeight(String text, float maxWidth) {
        ...
        float lineHeight = Fonts.pro[15].getFontHeight();
    ...
    private void drawSettingName(MatrixStack stack, String settingName, float x, float y) {
        Fonts.pro[15].drawString(stack, settingName, x, y + TEXT_Y_OFFSET, -1);
    }

    private void drawBackground(float x, float y, float width, float height) {
        RenderUtils.drawRoundedRect(x, y, width, height - 1, 4, ColorUtils.rgba(0, 0, 0, 128));
    }
